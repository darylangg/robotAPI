...
function preparePayload(payload, topic){
    ...
    if (topic == "SecurityCheckPoint"){
        ...
        if (ret["turnstileID"]){
            ret["doorName"] = [config.turnstiles[ret["turnstileID"]]+"_CONTROL"]
        } 
        if (ret["doorID"]){
        ...
        delete ret["open"]
        delete ret["close"]
    } else if (topic == "JtcLiftReservationRequest" || topic == "LiftSignalMessage"){
...
function buildExchangeName(protoType,topic){
    ...
    if (topic ==="TurnstileControlMessage" || topic ==="DoorControlMessage" || topic ==="LiftSignalMessage"){
