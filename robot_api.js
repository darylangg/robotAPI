...
const {keycloakGeneralConfig, keycloakGeneralAuthConfig} = require('./keycloak-config')
...
(async () => {
    ...
    server.route([
        {
            ...
            options: {
                ...
                handler: async (request, h) => {
                    amqp.sendProto(request.payload, "SecurityCheckpointRequest")
                ...
                validate: {
                    payload: Joi.object({
                        arrived : Joi.boolean().required().description("Whether robot is physically at security checkpoint"),
                        destination : Joi.array().items(
                            Joi.string().description("Destination Point")
                        ).required().description("List of destination points"),
        ...
        {   
            ...
            options: {
                auth: keycloakGeneralAuthConfig,
                handler: async (request, h) => {
                    const cached_data = replyCache.getCachedData("ListRobotsAtSecurityRequest")
                    var robot_list = []
                    if (cached_data){
        ...
        },
        {
            ...
            options: {
                auth:keycloakGeneralAuthConfig,
                handler: async (request, h) => {
                    amqp.sendProto(request.payload, "TurnstileAccessRequest")
                    const response = h.response({
                        "robotID":request.payload.robotID,
                        "message":"Received Request",
                        "timestamp": (new Date()).toISOString()
                ...
                validate: {
                    payload: Joi.object({
                        arrived : Joi.boolean().required().description("Whether robot is physically at holding zone"),
                        robotID : Joi.string().required().description("Robot ID"),
                        vendorID : Joi.string().required().description("Vendor ID"),
                        turnstileID : Joi.number().required().description("Turnstile ID"),
                        zone : Joi.string().valid("ENTER","EXIT").required().description("Enter or exit zone"),
                        timestamp : Joi.string().description("Timestamp in ISO 8601 format")
                    })
                }
            }
        },
        {
            ...
            options: {
                auth:keycloakGeneralAuthConfig,
                handler: async (request, h) => {
                    const cached_data = replyCache.getCachedData("ListRobotsAtTurnstileRequest")
                    const robot_list = []
                    if (cached_data){
        ...
        },
        {
            ...
            options: {
                auth: keycloakGeneralAuthConfig,
                handler: async (request, h) => {
                    const cached_data = replyCache.getCachedData("TurnstileCheckPermissionRequest")
                    var go = false
                    if (cached_data){
                        console.log(cached_data.approvedRobots[request.query.vendorID+"_"+request.query.robotID])
                        if (cached_data.approvedRobots[request.query.zone+"_"+request.query.vendorID+"_"+request.query.robotID] == request.query.turnstileID){
                            go = true
                        }
                    }
                    const data = {
        ...
        },
        {
            ...
            options: {
                auth: keycloakGeneralAuthConfig,
                handler: async (request, h) => {
                    var message = "Invalid Request - Open and close cannot have same values"
                    if (request.payload.open != request.payload.close){
                        message = "Received Request"
                        amqp.sendProto(request.payload, "TurnstileControlMessage")
                        amqp.sendProto(request.payload, "RPCRequest")

                        robotCache.setRobotTurnstile(request.payload.robotID, request.payload.vendorID, request.payload.turnstileID)
                    }
                    const response = h.response({
                        "robotID":request.payload.robotID,
                        "turnstile":request.query.turnstileID,
        ...
        },
        {
            ...
            options: {
                ...
                handler: async (request, h) => {
                    ...
                    var opened = null
                    if (cached_data){
                        var turnstileID = robotCache.getRobotTurnstile(request.query.robotID, request.query.vendorID)
                        turnstile = config.turnstiles[turnstileID]
                        const reading = cached_data["doorReading"].filter(reading=>reading.doorName == turnstile+"_CONTROL")[0]
        ...
        },
        {
            ...
            options: {
                auth: keycloakGeneralAuthConfig,
                handler: async (request, h) => {
                    var message = "Invalid Request - levels must be more than 0 and less than 31"
                    var boardingLevel = request.payload.boardingLevel
                    var destinationLevel = request.payload.destinationLevel

                    if (boardingLevel > 0 && boardingLevel <= 31 && destinationLevel >0 && destinationLevel <= 31){
                        message = "Received Request"
                        amqp.sendProto(request.payload,"LiftAccessRequest")
                    }
                ...
                validate: {
                    payload: Joi.object({
                        arrived : Joi.boolean().required().description("Whether robot is at holding zone"),
                        robotID : Joi.string().required().description("Robot ID"),
                        vendorID : Joi.string().required().description("Vendor ID"),
                        liftID : Joi.number().description("Preferred Lift"),
                        boardingLevel : Joi.number().required().description("Boarding Level"),
                        destinationLevel : Joi.number().required().description("Destination Level"),
                        timestamp : Joi.string().description("Timestamp in ISO 8601 format")
                    })
                }
            }
        },
        {
            ...
            options: {
                auth: keycloakGeneralAuthConfig,
                handler: async (request, h) => {
                    const cached_data = replyCache.getCachedData("ListRobotsAtLiftRequest")
                    var robot_list = []
        ...
        },
        {
            ...
            options: {
                auth: keycloakGeneralAuthConfig,
                handler: async (request, h) => {
                    const cached_data = replyCache.getCachedData("LiftCheckPermissionRequest")
                    var go = false
                    var liftID = null

                    var combined_id = request.query.vendorID+"_"+request.query.robotID
                    if (cached_data){
        ...
        },
        {
            ...
            options: {
                ...
                handler: async (request, h) => {
                    ...
                    if (boardingLevel > 0 && boardingLevel <= 31 && destinationLevel >0 && destinationLevel <= 31){
                        message = "Received Request"
                        amqp.sendProto(request.payload, "JtcLiftReservationRequest")
                    }
            ...
            options: {
                ...
                validate: {
                    payload: Joi.object({
                        ...
                        boardingLevel : Joi.number().required().description("Boarding Level"),
                        destinationLevel : Joi.number().required().description("Destination Level"),
                        embarkationStatus : Joi.number().required().description("Embarkation Status"),
                        timestamp : Joi.string().description("Timestamp in ISO 8601 format")
                    })
                }
            }
            ...
            options: {
                ...
                handler: async (request, h) => {
                    ...
                    if (currentLevel > 0 && currentLevel <= 31 ){
                        message = "Received Request"
                        amqp.sendProto(request.payload,"DoorAccessRequest")
                    }
        ...
        },
        {
            ...
            options: {
                auth: keycloakGeneralAuthConfig,
                handler: async (request, h) => {
                    const cached_data = replyCache.getCachedData("DoorCheckPermissionRequest")
                    var go = false
                    var door = null

                    if (cached_data){
                        door = cached_data.approvedRobots[request.query.zone+"_"+request.query.vendorID + "_" + request.query.robotID]
            ...
            options: {
                ...
                handler: async (request, h) => {
                    ...
                    if (request.payload.open != request.payload.close){
                        ...
                        robotCache.setRobotDoor(request.payload.robotID, request.payload.vendorID, request.payload.doorID)
                        robotCache.setRobotLevel(request.payload.robotID, request.payload.vendorID, request.payload.currentLevel)
                    }
        ...
        },
        {
            ...
            options: {
                ...
                handler: async (request, h) => {
                    ...
                    var doorID = null
                    if (cached_data){
                        doorID = robotCache.getRobotDoor(request.query.robotID, request.query.vendorID)
                        door = config.doors[doorID]
                        var reading = cached_data["doorReading"].filter(reading=>reading.doorName == door)[0]
                        
                        if (reading){
                            opened = (reading.status === 'OPEN')
                        }

                        level = robotCache.getRobotLevel(request.query.robotID, request.query.vendorID)
                    }
                    const data = {
                        "doorID":doorID,
                        "currentLevel": level,
    ...
    await server.start().then(console.log('Server running on %s', server.info.uri))
